...
RPI_TPU_MODEL="${RPI_TPU_MODEL:-/home/rpi5/repnext-pipeline/shrink_w48_20260615/repnext_m5_tanhgelu_real_full_192_logits_full_integer_quant_dwpatched_edgetpu.tflite}"
...
cd $RPI_WORK && $RPI_ENV video_segmentation_demo.py run \
  --input-frames input_frames \
  --output-frames rpi5_tpu_frames \
  --metrics rpi5_tpu_metrics.json \
  --name 'RPi5 + Coral TPU w48 INT8 192 (accuracy-invalid)' \
  --backend tflite \
  --delegate edgetpu \
  --model '$RPI_TPU_MODEL' \
  --size 192 \
...
mkdir -p "$OUT/rpi5_cpu_frames" "$OUT/rpi5_tpu_frames"
sshpass -p "$RPI_PASS" scp -o StrictHostKeyChecking=no -r "$RPI_HOST:$RPI_WORK/rpi5_cpu_frames/." "$OUT/rpi5_cpu_frames/"
sshpass -p "$RPI_PASS" scp -o StrictHostKeyChecking=no -r "$RPI_HOST:$RPI_WORK/rpi5_tpu_frames/." "$OUT/rpi5_tpu_frames/"
sshpass -p "$RPI_PASS" scp -o StrictHostKeyChecking=no "$RPI_HOST:$RPI_WORK/rpi5_cpu_metrics.json" "$OUT/"
sshpass -p "$RPI_PASS" scp -o StrictHostKeyChecking=no "$RPI_HOST:$RPI_WORK/rpi5_tpu_metrics.json" "$OUT/"

"$VIDEO_PY" "$ROOT/demo/make_wallclock_video_comparison.py" \
  --root "$OUT" \
  --output "$OUT/${NAME}_4track_wallclock_24fps.mp4" \
  --fps "$FPS" \
  --panel-width 360 \
  --panel-height 240 \
  --pad-seconds 1.0
...
python3 "$ROOT/demo/video_segmentation_demo.py" summarize \
  ...
  "$OUT/rpi5_tpu_metrics.json" \
  --out "$OUT/${NAME}_4track_wallclock_24fps_summary.md"
