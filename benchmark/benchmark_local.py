...
def main():
    ...
    if not args.skip_pytorch and Path(args.ckpt).exists():
        results.append(bench_pytorch_repnext(args.ckpt, args.size, args.warmup, args.runs, args.threads))
    if Path(args.tflite).exists():
        results.append(bench_tflite_cpu(args.tflite, args.warmup, args.runs, args.threads))
    if Path(args.edgetpu).exists():
        results.append(bench_edgetpu(args.edgetpu, args.warmup, args.runs, devices[0] if devices else None))
        if len(devices) >= 2:
            results.append(bench_edgetpu_data_parallel(args.edgetpu, args.warmup, args.runs, devices[:2]))
    if args.split_a and args.split_b:
        results.append(bench_edgetpu_pipeline(args.split_a, args.split_b, args.warmup, args.runs, devices[:2]))
